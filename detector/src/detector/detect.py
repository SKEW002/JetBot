...
class MoveBaseStateEnum(enum.Enum):
    PENDING = 0
    ACTIVE = 1
    PREEMPTED = 2
    SUCCEEDED = 3
...
class Detection:
    def __init__(self):
        ...
        '''Subscribers'''
        rospy.Subscriber(rospy.get_param("~input_image_topic"), Image, self.image_callback) # Get image

        '''Service'''
        self.frames_info_server = rospy.Service('/frame_info', FramesInfo, self.frames_info_callback) # end_effecter_pose camera frame

        self.type_of_object = 0
        ...
        '''Publisher'''
        self.detection_image_pub = rospy.Publisher(rospy.get_param("~output_image_topic"), Image, queue_size=10)
        self.bounding_boxes_pub = rospy.Publisher(rospy.get_param("~output_bboxes_topic"), BoundingBoxes, queue_size=1)
        ...
        self.bgr_colors = {'red':(0,0,255), 'green':(0,255,0), 'blue':(255,0,0), 'yellow':(0, 255, 217), 'orange':(0,140,255)}
